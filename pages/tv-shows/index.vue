      ...
      <div class="container mx-auto relative">
        <div class="flex w-full justify-between items-center mb-14">
          <div class="relative pt-3 inline-block">
            ...
            <h2 class="font-display text-2xl text-gray-100">TV Shows</h2>
          </div>
        </div>
        <div class="flex">
          <div class="w-60">
          ...
          <div class="flex-1 pl-8">
            <div v-if="tvShowsLoading" class="grid grid-cols-4 gap-x-6 gap-y-8 animate-pulse">
            ...
            <div
              v-if="tvShows.length > 0 && tvShowsLoading === false"
              class="grid grid-cols-4 gap-x-6 gap-y-8"
            >
              <CardMovie v-for="item in tvShows" :key="item.id" :data="item" :route="`tv-shows`" />
            </div>
...
export default {
  ...
  methods: {
    async getTVPopular() {
      ...
      try {
        const { data } = await this.$api.tvShows.getPopular(this.currentPage);
    ...
    async getGenre() {
      try {
        const { data } = await this.$api.general.getGenreTv();
